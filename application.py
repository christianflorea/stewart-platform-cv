...
class CameraVisionGUI:
    def __init__(self, root):
        ...
        self.controller_thread = threading.Thread(
            target=self.controller.run, args=(self.controller_callback,), daemon=True)
    ...
    def create_widgets(self):
        ...
        ttk.Button(
            Kp_buttons_frame,
            text="▲",
            command=lambda: self.change_value(PID.KP, self.controller_step)
        ...
        ttk.Button(
            Ki_buttons_frame,
            text="▲",
            command=lambda: self.change_value(PID.KI, self.controller_step / 10)
        ).pack(side="left", expand=True, fill="x", padx=(0, 2))
        ttk.Button(
            Ki_buttons_frame,
            text="▼",
        ...
        ttk.Button(
            Kd_buttons_frame,
            text="▲",
            command=lambda: self.change_value(PID.KD, self.controller_step)
        ).pack(side="left", expand=True, fill="x", padx=(0, 2))
        ttk.Button(
            Kd_buttons_frame,
            text="▼",
        ...
        ttk.Checkbutton(
            contour_frame,
            text="Show Platform Contour",
            variable=self.show_platform_var,
            command=self.update_show_platform
        ).pack(anchor="w", pady=2)
        self.show_ball_var = tk.BooleanVar(value=True)
        ttk.Checkbutton(
            contour_frame,
            text="Show Ball Contour",
            variable=self.show_ball_var,
            command=self.update_show_ball
        ).pack(anchor="w", pady=2)
        ...
        ttk.Button(
            button_frame,
            text=f"{'Stop' if self.cv.is_detecting() else 'Start'} Detection",
            command=self.toggle_detect_platform
        ).pack(fill="x", pady=2)
        ttk.Button(
            button_frame,
            text=f"{'Stop' if self.controller.is_active() else 'Start'} Path Follow",
            command=self.toggle_controller_following
        ).pack(fill="x", pady=2)
        ...
        self.ball_position_text.insert("1.0", "No data")
    ...
    def send_servo_commands(self, theta_1, theta_2, theta_3):
        ...
        if self.bus:
            try:
                self.bus.write_i2c_block_data(self.arduino_addr, 4, desired_angles)
            except Exception as e:
    ...
    def home_all_servos(self):
        if self.bus:
            try:
                self.bus.write_i2c_block_data(self.arduino_addr, 0x0, [0x0, 0x0])
    ...
    def update_frame(self):
        ...
        if frame is not None:
            ...
            plot_y = int(center_y + y_pid * scale)

            cv.circle(frame, (plot_x, plot_y), 10, (0, 255, 255), -1)
